...
- [Apache와 Nginx](#apache와-nginx)
  ...
  - [Apache HTTP Server의 개선 - Multi-Processing Module](#apache-http-server의-개선---multi-processing-module)
...
Apache는 UNIX 계열 OS가 네트워크 커넥션을 형성하는 모델을 사용했다. 다시말해, Apache는 HTTP 요청이 들어오면 커넥션을 형성하기 위해 프로세스를 생성한다.
...
컴퓨터의 보급이 빨라지고 웹이 발전함에 따라 웹 서버에 트래픽이 급속히 증가했다. 또한 웹 페이지의 렌더링 속도를 높이기 위해 웹 서버의 커넥션을 유지하는 기술이 도입됨에 따라 웹 서버가 감당해야할 커넥션의 수가 급속히 증가했다. 어느 날 웹 서버가 1만개 이상의 커넥션을 유지할 수 없는 현상이 발생했고 이러한 문제점을 **C10K**라 한다.

C10K 문제의 원인은 하드웨어가 아니었다. 문제의 원인은 Apache의 태생적 한계 때문이었다. Apache는 각 커넥션마다 프로세스를 생성하고 할당한다. 이로 인해 서버에서 유지해야 할 커넥션의 수가 많아지면서 메모리 부족현상을 야기했고, 컨텍스트 스위칭이 빈번하게 발생해 CPU의 부하가 커졌다.

Apache의 구조는 수 많은 커넥션을 감당하기에 적절하지 않았다. 이러한 Apache의 한계를 극복하기 위해 나타난 것이 Nginx이다

## Nginx의 등장
Nginx는 Apache가 많은 커넥션을 유지하지 못하는 단점을 해결하기 위한 보조 수단으로 탄생했다. Nginx가 Apache의 한계를 극복한 방법은 간단했다. Apache의 앞에 Nginx를 두어 클라이언트의 커넥션을 유지하고 정적 컨텐츠를 제공하고, 동적인 컨텐츠는 Apache가 제공하는 것이 었다.
...
이벤트는 커넥션 생성, 해제, 타임아웃, 에러 등 커넥션에서 발생하는 모든 것들을 의미한다. 커널은 Nginx에게 한 번에 많은 이벤트를 전달한다. Nginx의 각 스레드는 커널로 부터 전달받은 이벤트를 큐에 저장하고 하나씩 처리해 나간다.
...
Nginx의 스레드 풀은 태스크 큐와 작업을 처리하기 위한 스레드들로 이루어져 있다. 만약 Worker 프로세스가 잠재적으로 시간이 오래걸리는 명령이라고 판단하면, Worker 프로세스가 직접 작업을 처리하는 것이 아니라 스레드 풀의 태스크 큐에 작업을 전달한다. 그리고 Worker 프로세스는 바로 다음 이벤트를 처리하고, 스레드 풀의 유휴 스레드가 태스크 큐에서 작업을 꺼내 처리한다.

## Apache HTTP Server의 개선 - Multi-Processing Module
Apache도 구조적 단점을 개선하기 위해서 많은 변화가 이루어지고 있다. Apache의 가장 큰 특징인 모듈화된 설계를 통해 네트워크 커넥션을 형성하고 요청을 처리하기 위한 방식을 개선했는데 이를 다중처리 모듈(Multi-Processing Module, MPM)이라 한다.
...
Apache 2.4에서는 캐시, 프록시 모듈, 세션 제어, 비동기 읽기 및 쓰기 지원 기능과 같은 성능 개선을 최우선으로 삼았다. 이는 최근 웹 서버를 정적 데이터 처리를 통한 속도 향상을 노리는 프록시나 캐시 처리용 서버로 사용하는 경향이있기 때문이다.
